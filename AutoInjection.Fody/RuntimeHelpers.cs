...
namespace Cecilifier.Runtime
{
    public class TypeHelpers
    {
        ...
        public static MethodInfo ResolveGenericMethod(string declaringTypeName, string methodName, BindingFlags bindingFlags, IEnumerable<TypeReference> typeArguments, IEnumerable<ParamData> paramTypes)
        {
            ...
            foreach (var mc in methods)
            {
                ...
                if (found)
                {
                    return mc.MakeGenericMethod(typeArguments.Select(v =>
                    {
                        // var name = $"{v.FullName}, {v.Module.Assembly.FullName}";
                        // Above include version, but do i need to?
                        var name = $"{v.FullName}, {v.Module.Assembly.FullName}";
                        var t = Type.GetType(name);
                        if (t == null)
                        {
                            throw new NullReferenceException($"Not found {name}");
                        }
                        return t;
                    }).ToArray());
                }
        ...
        {
            var declaringType = Type.GetType(declaringTypeName, true);
